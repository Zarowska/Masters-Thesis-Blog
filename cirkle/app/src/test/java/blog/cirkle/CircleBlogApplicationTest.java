...
class CircleBlogApplicationTest {
	...
	void loadContext() throws IOException {
		...
		assertTrue(uploadAndDownloadFile(apiClient, "test-data/cirkle.jpg").isPresent());
	...
	private Optional<byte[]> uploadAndDownloadResizedFile(ApiClient apiClient, String imageName, Integer height,
		...
		UUID imageId = UUID.fromString(uploadUrl.substring(uploadUrl.lastIndexOf('/') + 1));
