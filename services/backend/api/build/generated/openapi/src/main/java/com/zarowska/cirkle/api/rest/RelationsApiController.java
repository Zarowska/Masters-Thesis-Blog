...
public class RelationsApiController implements RelationsApi {
    ...
    private final RelationsApiDelegate delegate;

    public RelationsApiController(@Autowired(required = false) RelationsApiDelegate delegate) {
        this.delegate = Optional.ofNullable(delegate).orElse(new RelationsApiDelegate() {});
    }

    @Override
    public RelationsApiDelegate getDelegate() {
        return delegate;
    }
