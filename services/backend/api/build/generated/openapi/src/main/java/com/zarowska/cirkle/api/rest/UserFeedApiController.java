...
public class UserFeedApiController implements UserFeedApi {

    private final UserFeedApiDelegate delegate;

    public UserFeedApiController(@Autowired(required = false) UserFeedApiDelegate delegate) {
        this.delegate = Optional.ofNullable(delegate).orElse(new UserFeedApiDelegate() {});
    }

    @Override
    public UserFeedApiDelegate getDelegate() {
        return delegate;
    }
