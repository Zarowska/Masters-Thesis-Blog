...
public class PostCommentsApiTest extends AbstractTest {
    ...
    void shouldCreatePostComment() throws Exception {
        context("Bob Marley", "bob@marley.com", "http:/some/avatar").apply(ctx -> {
            ...
            CreateCommentRequest commentRequest = CreateCommentRequest.builder().text("New comment").images(Collections.emptyList())
                    .build();

            Optional<Comment> newComment = ctx.getApi().posts().comments().addPostComment(ctx.getUserId(),newPost.get().getId(),commentRequest);
